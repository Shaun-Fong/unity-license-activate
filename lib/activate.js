...
function getHost(email) {
  ...
  if (email.includes('@163.com')) return 'imap.163.com';
  if (email.includes('@outlook.com')) return 'outlook.office365.com';
  return null;
}
...
async function getVerification(email, password, count = 0) {
  ...
  try {
    ...
    parse(email, password, 993, true, savePath);
...
async function start(email, password, alf, verification, emailPassword) {
  ...
  try {
    ...
    console.log(`[INFO] Downloading ulf file at ${downloadPath} ...`);
